...
function bookingCheckinMin(b) {
    ...
    // same booking — keep this in step with dashboard.html, havens.html, payment.html,
    // todaysbooking.html and nicole-dashboard.html. This copy said 9:00 AM and split the two.
    if (h === 6) return parseTimeMin(b.slot === "evening" ? "7:00 PM" : "8:00 AM");
    if (h === 21) return parseTimeMin(havenTimeIn(b.haven) || "12:00 NN");
...
function quickAnswer(text, d, opts, today) {
    ...
    if (_has(t, "collect", "collected", "kita", "kinita", "kitain", "kikitain", "kinikita", "kikita", "nakolekta", "sales", "revenue", "income", "takings", "benta", "kinabenta", "napagbentahan", "earnings", "earn", "earned")
        && (_has(t, "how much", "magkano", "total", "much", "gaano", "ilan", "today", "ngayon") || _has(t, "collect", "collected", "nakolekta", "kita", "kinita", "kitain", "kikitain", "kinikita"))) {
    ...
    // DEPOSITS — security deposits to return ("magkano ibabalik na deposit today")
    if(_has(t, "deposit", "deposits", "security deposit", "ibabalik", "ibalik", "i-refund", "refund", "bond", "piyansa", "paunang bond")) {
        const w = when || { from: today, to: today, label: "today", single: true };
        const outs = L.filter(b => { const o = b.checkout || endExcl(b); return o >= w.from && o <= w.to && (Number(b.deposit) || 0) > 0; });
        if(!outs.length) return { reply: `No security deposits to return ${w.label} — no check-outs with a deposit held.`, tools: ["list_bookings"] };
        const due = outs.filter(b => !b.depositReturned);
        const total = due.reduce((s, b) => s + (Number(b.deposit) || 0), 0);
        const lines = outs.sort((a, b) => parseTimeMin(effectiveCheckoutStr(a)) - parseTimeMin(effectiveCheckoutStr(b))).map(b =>
            `• ${primaryName(b)} — ${b.haven}, ${peso(b.deposit)}` + (b.depositReturned ? " (already returned ✓)" : ` (out ${effectiveCheckoutStr(b)})`));
        const head = due.length
            ? `${due.length} deposit${due.length === 1 ? "" : "s"} to return ${w.label} — ${peso(total)} total`
            : `All deposits for ${w.label} are already returned`;
        return { reply: `${head}:\n${lines.join("\n")}`, tools: ["list_bookings"] };
    }

    // OVERVIEW / STATUS — the catch-all for friendly, general questions like
    // "kamusta bookings natin", "how are we doing", "status", "anong nangyayari".
    // Placed last so specific intents win first; this keeps vague questions FREE
    // instead of dropping them to the paid AI (or the no-key message).
    if(_has(t, "kamusta", "kumusta", "overview", "summary", "status", "buod", "kabuuan", "update", "balita",
        "how are we", "how are things", "how are our", "how's our", "hows our", "how is our", "how's business", "hows business", "how's it going",
        "anong nangyayari", "ano nangyayari", "anong meron", "how's everything", "hows everything", "how are the bookings", "recap", "briefing",
        "how we doing", "kamusta tayo", "kumusta tayo")) {   // NOTE: no bare "today"/"ngayon" here — they'd swallow specific questions
        const arr = L.filter(b => b.checkin === today);
        const dep = L.filter(b => (b.checkout || endExcl(b)) === today);
        const inH = L.filter(b => b.checkin <= today && today < endExcl(b));
        const occ = [...new Set(inH.map(b => b.haven))].length, total = havenNames(d).length;
        const lines = [
            `${arr.length} checking in, ${dep.length} checking out today.`,
            `${inH.length} stay${inH.length === 1 ? "" : "s"} in house — ${occ}/${total} haven${total === 1 ? "" : "s"} occupied.`
        ];
        if(opts.canSeeMoney){
            const m = moneySummary(d, today, today);
            const owed = inH.reduce((s, b) => s + balanceOf(b), 0);
            lines.push(`${peso(m.collected)} collected today${owed > 0 ? `, ${peso(owed)} still owed by guests in house` : ""}.`);
        }
        let unsold = 0;
        for(const hv of havenNames(d)) unsold += openingsFor(d, hv, today).length;
        if(unsold > 0) lines.push(`${unsold} haven${unsold === 1 ? " has" : "s have"} sellable open hours today — worth filling.`);
        return { reply: `Here's where things stand today:\n${lines.map(l => "• " + l).join("\n")}\n\nAsk me for the details on any of these — arrivals, who owes you, what's free, and so on.`, tools: ["daily_briefing"] };
    }
...
async function ask({ messages, user, admin }) {
    ...
    // Not a common question. With no key we stay in free mode — never nag about an
    // API key in the chat; just say we didn't catch it and show what we can do.
    if (!process.env.ANTHROPIC_API_KEY) {
        return {
            ok: true, source: "quick", free: true, cost_php: 0,
            reply: "Hmm, I didn't quite catch that one. " + quickHelp(opts)
        };
    }
...
module.exports = {
    ask, getBrief, checkAvailability, enabled: () => !!process.env.ANTHROPIC_API_KEY,
    // The booking time maths, shared so nothing has to copy it AGAIN (there are already too many
    // copies — dashboard/todaysbooking/nicole). The guest-guide gate (/stay/:token in server.js)
    // uses bookingInterval to decide whether a stay is happening right now. Both return ABSOLUTE
    // minutes since the epoch anchored to Manila (+08:00), so they compare directly with
    // Date.now()/60000 no matter what zone the server runs in.
    bookingCheckinMin,   // minutes past midnight of the check-in DATE (0 is coerced to 1440 = next day)
    bookingInterval      // { start, end } in absolute minutes
};
